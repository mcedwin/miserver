...
El botón **Pull** ejecuta el comando `app:pull` del wrapper, que sincroniza el
repositorio con `git fetch --depth 1` + `git checkout -f -B <rama> origin/<rama>`
(descartando cambios locales tracked) y, si tiene éxito, corre el despliegue
(Composer + npm + caches) en la misma tarea.
